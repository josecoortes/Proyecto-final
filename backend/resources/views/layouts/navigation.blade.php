<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('admin.platos.index')" :active="request()->routeIs('admin.platos.*')">
                        {{ __('Gestión de Platos') }}
                    </x-nav-link>
                    @endif

                    @if(in_array(Auth::user()->rol, ['admin', 'repartidor', 'cajero']))
                    <x-nav-link :href="route('admin.pedidos.index')" :active="request()->routeIs('admin.pedidos.*')">
                        {{ __('Logística / Pedidos') }}
                    </x-nav-link>
                    @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(in_array(Auth::user()->rol, ['admin', 'empleado']))
            <x-responsive-nav-link :href="route('admin.platos.index')" :active="request()->routeIs('admin.platos.*')">
                {{ __('Gestión de Platos') }}
            </x-responsive-nav-link>
            @endif
            ...
            @if(Auth::user()->rol === 'admin')
            <x-responsive-nav-link :href="route('admin.empleados.index')" :active="request()->routeIs('admin.empleados.*')">
                {{ __('Empleados') }}
            </x-responsive-nav-link>
            @endif
        </div>
